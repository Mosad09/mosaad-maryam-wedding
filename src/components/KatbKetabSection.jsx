...
// Dedicated layout (not the generic EventSection) so the photo shows in
// full via a framed, uncropped presentation — same treatment as the
// engagement photo. No location/map shown here by design — the map only
// lives in VenueSection.
export default function KatbKetabSection() {
  const { pick } = useLanguage();
  const event = weddingData.events.katbKetab;

  return (
    <section id="katb-ketab" className="py-20 px-6">
